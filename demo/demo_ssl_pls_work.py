...
def get_ssl_model(args, num_classes):
    def _get_state_dict(ssl_method):
        if ssl_method == 'dino':
            state_dict = load_dino_weights(checkpoint_location=args.checkpoint_dir)
        elif ssl_method == 'byol':
            state_dict = load_byol_weights(checkpoint_location=args.checkpoint_dir)
        elif ssl_method == 'moco':
            state_dict = load_moco_weights(checkpoint_location=args.checkpoint_dir)
        elif ssl_method == 'barlow_twins':
            state_dict = load_barlow_twins_weights(checkpoint_location=args.checkpoint_dir)
        return state_dict

    model = torchvision.models.detection.fasterrcnn_resnet50_fpn(pretrained=False, pretrained_backbone=False)

    # Initialize backbone with self supervised weights
...
def main(args):
    ...
    if  args.ssl_method is None:
        ...
        model = get_ssl_model(args, num_classes)
